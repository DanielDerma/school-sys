...
export default function GroupedSelect({ data, changeRadarParms }) {
  return (
    <Box>
      ...
      >
        <EnhancedTable data={data} changeRadarParms={changeRadarParms} />
      </Box>
    </Box>
  );
}
...
function EnhancedTable({ data, changeRadarParms }) {
  const [order, setOrder] = useState("asc");
  const [orderBy, setOrderBy] = useState("calories");
  const [selected, setSelected] = useState([]);

  useEffect(() => {
    changeRadarParms(selected);
  }, [selected, changeRadarParms]);
